...
def annualaverage(admno,session,arm,klass,subject):
    ...
    for j in acaderec:
        if SubjectScore.objects.filter(academic_rec = j,klass = klass,subject = subject,session = session):
            ...
            if float(totsubject1.term_score) <= 0 :
                pass
            else:
                summ = summ + totsubject1.term_score
                tcount  += 1
        else:
            pass
    if tcount == 0:
...
def classposition(session,term,klass,arm):
    if term == 'Third':
        ...
        for d in flist:
            pos = ordinal(n)
            StudentAcademicRecord.objects.filter(term = term,session = session,klass = klass,percentage = d).update(position = pos)
            j = stuperc.count(d)
            n  += j
            #end of position
    else:
        ...
        for d in flist:
            pos = ordinal(n)
            StudentAcademicRecord.objects.filter(term = term,session = session,klass = klass,arm = arm,percentage = d).update(position = pos)
            j = stuperc.count(d)
            n += j
    return 'ok'
...
def classposition1(session,term,klass):
    if term == 'Third':
        ...
        for d in flist:
            pos = ordinal(n)
            StudentAcademicRecord.objects.filter(term = term,session = session,klass = klass,percentage = d).update(position1 = pos)
            j = stuperc.count(d)
            n  += j
            #end of position
    else:
...
def subjectposition(session,subject,term,klass,arm):
    if term == 'Third':
        ...
        for t in dflist:
            spos = ordinal(b)
            SubjectScore.objects.filter(klass = klass,arm = arm,subject = subject,session = session,term = term,term_score = t).update(subposition = spos)
            f = stusubp.count(t)
            b += f
    return 'ok'
...
def mid_term_subjectposition(session,subject,term,klass,arm,stuper):
        ...
        stusubp = []
        for h in subpli:
            stusubp.append(h.mid_term)
            #stusubp.append(h.first_ca)
        stusubp.sort(reverse=True)
        dicsubp = {}
        for n in stusubp:
            ddic = {n:n}
