                ...
                for j in Student.objects.filter(admitted_session = session,admitted_class = klass,admitted_arm = arm,gone = False):
                    if StudentAcademicRecord.objects.filter(student = j,session = session,term = term):
                        ...
                        if SubjectScore.objects.filter(academic_rec = st,klass = klass,subject = subject,session = session,arm=arm,term =term):
                            ...
                            kk = {'id':gs.id,'admissionno':j.admissionno,'fullname':j.fullname,'sex':j.sex,'subject':gs.subject,'term':str(term),'first_ca':gs.first_ca,'second_ca':gs.second_ca,'exam_score':gs.exam_score}
                            stlist.append(kk)
                        else:
                            pass
    ...
    if  "userid" in request.session:
        if request.is_ajax():
            if request.method == 'POST':
                ...
                #    subjectlist = Subject.objects.filter(category = 'JS').order_by('num')
                #    fs = {}
                ...
                for j in stuinfo:
                        if StudentAcademicRecord.objects.filter(student = j,term = term):
                            ...
                            for uuu in Subject.objects.all():
                                submid = uuu.ca
                            submiddiv = int(submid)/2#divide total CA by 2
                            msublist = []
                            for jj in subsco:
                                fca = jj.first_ca
                                sca = jj.second_ca
                                ...
                                if klass[0] == 'S':
                                    rem = seniorgrade(float(totalperc))
                                elif klass[0] == 'N' or klass[0] == 'C' or klass[0] == 'L' or klass[0] == 'B' or klass[0] == 'Y' or klass[0] == 'P':
                                    rem = prygrade(float(totalperc))
                                else:
                                    rem = juniorgrade(float(totalperc))
                                tscore += totalperc
                                print rem
                                msub = {'subject':jj.subject,'first_ca':fca,'second_ca':sca,'totalca':totalca,'totalperc':locale.format("%.1f",totalperc,grouping=True),'remark':rem['remark'],'grade':rem['grade'],'teacher':jj.subject_teacher,'position':jj.subposition}
                                msublist.append(msub)
                            stuper = tscore / totalmarkcount
                            #**********************************************
                            jdic = {'studentinfo':j,'academic':acaderec,'affective':affskill,'pyscho':psycho,'subject':msublist,'totalmark':locale.format("%.1f",tscore,grouping=True),'totalmark1':rtotalsec,'totalmark2':rtotalsec1,'annualavg':rtotal4,'getgrading':getgrading,'percentage':locale.format("%.2f",stuper,grouping=True)}
                            replist.append(jdic)
                if classtot == 0 or stuno == 0:
                        clavg = 0.0
                else:
...
def primary_url(request,session1,klass1,arm1,name1,term1):
        ...
        if StudentAcademicRecord.objects.filter(student = getstu,term = term):
            comm = StudentAcademicRecord.objects.get(student = getstu,term = term)
            getdetails = SubjectScore.objects.filter(session = session,klass = klass, arm = arm,term = term,academic_rec = comm).order_by('num')
        return render_to_response('assessment/subjectpry1.html',{'getdetails':getdetails,'session':session,'klass':klass,'arm':arm})
    ...
    if  "userid" in request.session:
        ...
        for j in Student.objects.filter(admitted_session = session,admitted_class = klass,admitted_arm = arm,gone = False).order_by('fullname'):
            if StudentAcademicRecord.objects.filter(student = j,session = session,term = term):
                ...
                if SubjectScore.objects.filter(academic_rec = st,klass = klass,subject = subject,session = session,arm=arm,term =term):
                    gs = SubjectScore.objects.get(academic_rec = st,klass = klass,subject = subject,session = session,arm=arm,term =term)
                    kk = {'id':gs.id,'admissionno':j.admissionno,'fullname':j.fullname,'sex':j.sex,'subject':gs.subject,'term':str(term),'first_ca':gs.first_ca,'second_ca':gs.second_ca,'exam_score':gs.exam_score,'termscore':gs.term_score,'remark':gs.grade,'position':gs.subposition}
                    stlist.append(kk)
                else:
                    pass
