...
class tblsetup(models.Model):
    ...
    class Meta:
        ordering =['name']
        verbose_name_plural = "SETUP TRAINING"
...
class tbltraining(models.Model):
    ...
    def __unicode__(self):
        return  u'%s %s %s %s %s %s %s %s %s %s %s' % (self.staffid,self.staffname,self.sex,self.designation,self.department,self.traintype,self.description,self.duration,self.commdate,self.userid,self.recyear)
    class Meta:
        ordering =['staffid','id']
